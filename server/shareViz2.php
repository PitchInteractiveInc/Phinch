...
$q = 'INSERT INTO SharedData (biom_filename, biom_file_server_hash, ip_address, from_email, from_name, to_email, to_name, notes, url_hash, visualization_id, layer_id, visualization_options, date_uploaded, countView, filter_options_json) VALUES ';
$q .= '(:biom_filename, :biom_file_server_hash, :ip_address, :from_email, :from_name, :to_email, :to_name, :notes, :url_hash, :visualization_id, :layer_id, :visualization_options, NOW(), 0, :filter_options_json)';
$stmt = $db->prepare($q);
...
//calc file hash
$fileHash = md5($_POST['biomFile']);
//see if hash is in DB
$biomExists = $db->prepare('SELECT biom_filename FROM SharedData WHERE biom_file_server_hash = :biom_file_hash');
$biomExists->bindParam(':biom_file_hash', $fileHash);
$biomExists->execute();
$exists = $biomExists->fetch(PDO::FETCH_OBJ);

$stmt->bindParam(':biom_file_server_hash', $fileHash);
...
if(! $exists) {
	...
	$stmt->bindParam(':biom_filename', $exists->biom_filename);
}
